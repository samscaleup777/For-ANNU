(()=>{
  ...
  const boot=()=>{
    const css=document.createElement('link');css.rel='stylesheet';css.href='/chatbot.css?v=2';document.head.appendChild(css);
    ...
    const bindDrag=el=>{el.addEventListener('pointerdown',ev=>{if(ev.button!==undefined&&ev.button!==0)return;const startX=ev.clientX,startY=ev.clientY,rect=root.getBoundingClientRect();let moved=false;el.setPointerCapture?.(ev.pointerId);const move=e=>{const dx=e.clientX-startX,dy=e.clientY-startY;if(Math.abs(dx)+Math.abs(dy)>4)moved=true;if(!moved)return;root.style.left=clamp(rect.left+dx,8,window.innerWidth-76)+'px';root.style.top=clamp(rect.top+dy,8,window.innerHeight-76)+'px';root.style.right='auto';root.style.bottom='auto';if(state.open)applyShellSide()};const up=()=>{state.dragMoved=moved;if(moved){savePosition();setTimeout(()=>state.dragMoved=false,80)}el.releasePointerCapture?.(ev.pointerId);el.removeEventListener('pointermove',move);el.removeEventListener('pointerup',up)};el.addEventListener('pointermove',move);el.addEventListener('pointerup',up)})};
