(()=>{
  ...
  const boot=()=>{
    ...
    const root=document.createElement('div');root.className='annika-chat-root';root.innerHTML=''+
      ...
      '<textarea class="annika-chat-input" rows="1" maxlength="3000" placeholder="Ask Annïka anything…" aria-label="Message Annïka"></textarea>'+
    ...
    const addBubble=(text,who='bot',sources=[])=>{
      const row=document.createElement('div');row.className='annika-chat-row '+who;
      const bubble=document.createElement('div');bubble.className='annika-chat-bubble';bubble.textContent=text;row.appendChild(bubble);
      if(Array.isArray(sources)&&sources.length&&who==='bot'){
        const sourceBox=document.createElement('div');sourceBox.className='annika-chat-sources';
        const label=document.createElement('div');label.className='annika-chat-sources-label';label.textContent='Sources';
        sourceBox.appendChild(label);
        sources.slice(0,4).forEach(source=>{
          const a=document.createElement('a');a.className='annika-chat-source';a.href=source.url;a.target='_blank';a.rel='noopener noreferrer';a.textContent=source.title||source.url;sourceBox.appendChild(a);
        });
        row.appendChild(sourceBox);
      }
      messages.appendChild(row);messages.scrollTop=messages.scrollHeight;return bubble
    };
    const addWelcome=()=>{const row=document.createElement('div');row.className='annika-chat-row bot';const bubble=document.createElement('div');bubble.className='annika-chat-bubble';bubble.innerHTML='<div class="annika-chat-welcome"><div class="annika-chat-kicker">For faith. For confidence. For life.</div><h4>Ask me anything.</h4><p>I can answer general questions, explain topics, help you explore Annïka, and verify current or specific facts when web search is available.</p></div>';row.appendChild(bubble);messages.appendChild(row)};
    ...
    const ask=async text=>{const message=(text||'').trim();if(!message||state.busy)return;addBubble(message,'user');state.history.push({role:'user',content:message});setBusy(true);input.value='';input.style.height='auto';const typing=showTyping();typing.innerHTML='<span class="annika-chat-typing"><span></span><span></span><span></span></span>';try{const response=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message,history:state.history.slice(-10),page:window.location.pathname})});const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||'Chat service unavailable');const answer=(data.answer||'I’m sorry, I could not answer that just now. Please try again.').trim();replaceTyping(typing,answer);
      ...
      state.history.push({role:'assistant',content:answer})}catch(error){replaceTyping(typing,'I’m having trouble connecting right now. Please try again in a moment.')}finally{setBusy(false);input.focus()}};
    ...
    const bindDrag=el=>{let active=false,startX=0,startY=0,baseLeft=0,baseTop=0;
      ...
      const onUp=()=>{if(!active)return;active=false;window.removeEventListener('pointermove',onMove);window.removeEventListener('pointerup',onUp);window.removeEventListener('pointercancel',onUp);if(state.dragMoved){savePosition();setTimeout(()=>state.dragMoved=false,120)}};
      el.addEventListener('pointerdown',ev=>{if(ev.button!==undefined&&ev.button!==0)return;active=true;const rect=root.getBoundingClientRect();startX=ev.clientX;startY=ev.clientY;baseLeft=rect.left;baseTop=rect.top;el.setPointerCapture?.(ev.pointerId);window.addEventListener('pointermove',onMove);window.addEventListener('pointerup',onUp);window.addEventListener('pointercancel',onUp);ev.preventDefault()});
    };
