document.addEventListener('DOMContentLoaded',()=>{
  ...
  // Fresh animation stylesheet without hiding the page while it loads.
  const oldAni=[...document.querySelectorAll('link[href^="/animations-v3.css"]')];
  oldAni.forEach(link=>{link.href='/animations-v3.css?v=4';});
  if(!oldAni.length){
    const animationStyle=document.createElement('link');
    animationStyle.rel='stylesheet';animationStyle.href='/animations-v3.css?v=4';
  ...
  const visualMap={
    'page-home':['/assets/section-fashion.svg','/assets/section-faith.svg','/assets/section-safety.svg','/assets/section-lifestyle.svg'],
    'page-quran':['/assets/section-faith.svg','/assets/section-faith.svg','/assets/section-faith.svg','/assets/section-lifestyle.svg'],
    'page-safety':['/assets/section-safety.svg','/assets/section-safety.svg','/assets/section-safety.svg','/assets/section-faith.svg'],
    'page-lifestyle':['/assets/section-lifestyle.svg','/assets/section-lifestyle.svg','/assets/section-faith.svg','/assets/section-safety.svg'],
    'page-blog':['/assets/section-blog.svg','/assets/section-blog.svg','/assets/section-faith.svg','/assets/section-lifestyle.svg'],
    'page-about':['/assets/section-lifestyle.svg','/assets/section-faith.svg','/assets/section-safety.svg','/assets/section-blog.svg'],
    'page-contact':['/assets/section-safety.svg','/assets/section-blog.svg','/assets/section-lifestyle.svg','/assets/section-faith.svg']
  };
  const visuals=visualMap[pageClass]||visualMap['page-home'];

  // Add a relevant local image treatment to content cards that do not already contain media.
  document.querySelectorAll('.grid4 .card').forEach((card,index)=>{
    if(card.querySelector('img,.context-media'))return;
    const src=visuals[index%visuals.length];
    const media=document.createElement('div');media.className='context-media';
    const img=document.createElement('img');img.src=src;img.alt='Annïka visual for '+((card.querySelector('h3')?.textContent||'this topic').trim());img.loading='lazy';
    media.appendChild(img);card.prepend(media);card.classList.add('has-context-media');
  });

  const feature=document.querySelector('.feature-photo');
  if(feature&&!feature.querySelector('img')){
    const featureImages={'page-home':'/assets/section-lifestyle.svg','page-quran':'/assets/section-faith.svg','page-safety':'/assets/section-safety.svg','page-lifestyle':'/assets/section-lifestyle.svg','page-blog':'/assets/section-blog.svg','page-about':'/assets/section-blog.svg','page-contact':'/assets/section-safety.svg'};
    feature.style.backgroundImage=`linear-gradient(135deg,rgba(18,49,38,.08),rgba(18,49,38,.32)),url('${featureImages[pageClass]||'/assets/section-lifestyle.svg'}')`;
    feature.setAttribute('aria-label','Annïka section visual');
  }
  ...
  const revealTargets=[...document.querySelectorAll('.section-head,.card,.panel,.feature-photo,.cta-box,.footer,.legal article,.contact-card,.fashion-page .topic-card,.fashion-video-block,.howto-card')];
  ...
  if(reduced||!('IntersectionObserver' in window))revealAll();
    ...
    revealTargets.forEach((el,index)=>{el.style.setProperty('--reveal-delay',`${(index%4)*150}ms`);observer.observe(el);});
    // Mobile browsers occasionally delay the first intersection callback; reveal visible items as a safe fallback.
    requestAnimationFrame(()=>{
      const vh=window.innerHeight||800;
      revealTargets.forEach(el=>{if(el.getBoundingClientRect().top<vh*.92)el.classList.add('is-visible');});
    });
    setTimeout(()=>revealTargets.forEach(el=>{if(el.getBoundingClientRect().top<window.innerHeight*1.2)el.classList.add('is-visible');}),900);
  }

  // Fashion media is always first-party and repo-local.
  ...
  if(['page-fashion','page-quran','page-safety','page-lifestyle'].includes(pageClass)){
    ...
    if(anchor&&!document.querySelector('.ani-library-strip')){
      const strip=document.createElement('div');strip.className='ani-library-strip wrap';
      const track=document.createElement('div');track.className='ani-library-track';
      const labels=pageClass==='page-fashion'?['Hijab','Abaya','Burkha','Wardrobe','Fabric','Layering']:pageClass==='page-quran'?['Read','Reflect','Verify','Context','Learn','Practice']:pageClass==='page-safety'?['Privacy','Travel','Boundaries','Emergency','Accounts','Awareness']:['Prayer','Wellbeing','Family','Learning','Ramadan','Rest'];
      [...labels,...labels].forEach(label=>{const chip=document.createElement('span');chip.textContent=label;track.appendChild(chip);});
      strip.appendChild(track);anchor.parentNode.insertBefore(strip,anchor.nextSibling);
    }
  }
});
